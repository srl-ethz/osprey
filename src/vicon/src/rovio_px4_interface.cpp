...
class RovioPX4Interface : public rclcpp::Node
{
private:
  // ros
  ...
  rclcpp::TimerBase::SharedPtr timer_slam_tf_;
  ...
  */
  ...
  /**
   * @brief Transform a pose from Rovio's camera convention (C->W_ENU) to ned (B_NED->W_NED).
   * 
  ...
  */
  geometry_msgs::msg::PoseStamped transform_camera_to_ned(geometry_msgs::msg::PoseStamped pose_in) 
  {
    ...
    // print pose_in with euler angles for debugging
    tf2::Quaternion q_in;
    tf2::fromMsg(pose_in.pose.orientation, q_in);
    ...
    RCLCPP_INFO(this->get_logger(), 
                "VIO->PX4: x: %f, y: %f, z: %f, roll: %f, pitch: %f, yaw: %f",
    ...
    // Require: W_ENU->W_NED and C->D_NED
    ...
    tf2::Quaternion q_ned_enu, q_ned_C;
    tf2::fromMsg(t_ned_nwu.transform.rotation, q_ned_enu);
    tf2::fromMsg(t_ned_C.transform.rotation, q_ned_C);

    // transform rotation
    tf2::Quaternion q_Wned_Bned;
    tf2::fromMsg(pose_in.pose.orientation, q_Wned_Bned);
    q_Wned_Bned = convert_rot_camera_to_ned(q_Wned_Bned, q_ned_enu, q_ned_C.inverse());
    pose_out.pose.orientation = tf2::toMsg(q_Wned_Bned);

    // transform translation from ENU to NED (switch x and y axes, inverse z)
  ...
  */
  ...
  void receive_slam_tf()
  {
    // get transform from frame "world" to "camera0"
    geometry_msgs::msg::TransformStamped transform;
    try {
        transform = tf_buffer_->lookupTransform("world", "camera0", rclcpp::Time(0));
        // convert to NED

        // transform Transform to PoseStamped
        geometry_msgs::msg::PoseStamped pose_in = geometry_msgs::msg::PoseStamped();
        pose_in.header.frame_id = transform.header.frame_id;
        pose_in.header.stamp = transform.header.stamp;
        pose_in.pose.position.x = transform.transform.translation.x;
        pose_in.pose.position.y = transform.transform.translation.y;
        pose_in.pose.position.z = transform.transform.translation.z;
        pose_in.pose.orientation = transform.transform.rotation;

        geometry_msgs::msg::PoseStamped pose = transform_camera_to_ned(pose_in);
        // adjust translation to NED frame
        ...
        // convert to vision estimate and send to PX4
    ...
    // // print for debug
    // RCLCPP_INFO(this->get_logger(), 
    //             "world->camera0: x: %f, y: %f, z: %f, qx: %f, qy: %f, qz: %f, qw: %f",
    ...
    // print pose for debugging, convert rotation to Euler angles
    // tf2::Quaternion q;
    // tf2::fromMsg(pose.pose.orientation, q);
    // tf2::Matrix3x3 m(q);
    // double roll, pitch, yaw;
    // m.getRPY(roll, pitch, yaw);
    // RCLCPP_INFO(this->get_logger(), 
    //             "NED Pose: x: %f, y: %f, z: %f, roll: %f, pitch: %f, yaw: %f",
...
public:
  RovioPX4Interface(std::shared_ptr<Telemetry> telemetry, std::shared_ptr<Mocap> vision)
  : Node("vio_px4_interface"), telemetry_(telemetry), vision_(vision)
  {
    ...
    //   slam_raw_topic_, 10, std::bind(&RovioPX4Interface::odometry_callback, this, std::placeholders::_1));
    ...
    timer_px4_pub_ = this->create_wall_timer(std::chrono::milliseconds(10), 
                                         std::bind(&RovioPX4Interface::stream_px4_pose, this));

    int slam_fps = 30;
    timer_slam_tf_ = this->create_wall_timer(std::chrono::milliseconds(1000 / slam_fps), 
                                         std::bind(&RovioPX4Interface::receive_slam_tf, this));
...
int main(int argc, char *argv[]) 
{
  ...
  auto node = std::make_shared<RovioPX4Interface>(telemetry, vision);
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), 
      "RovioPX4Interface ready subscribing to /slam/odometry.");
